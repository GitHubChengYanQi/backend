<template>
  <div>
    ...
    <a-table
      ...
      <div slot="uricAcid" slot-scope="text">
        {{ returnTableText(text) }}
      </div>
...
import { uricAcidTestApi, exportUricAcidTestApi } from '@/api/healthTest'
...
export default {
    ...
    return {
      ...
      tableColumns: [
        ...
        },
        {
          ...
          align: 'center',
          width: 200,
          scopedSlots: { customRender: 'uricAcid' }
        },
        {
          title: '数据来源',
  ...
  methods: {
    ...
    exportData () {
      ...
      params.isMemberCenterUser = params.isMemberCenterUser === '-1' ? '' : params.isMemberCenterUser
      exportUricAcidTestApi(params).then(response => {
        this.tableLoading = false
        callDownLoadByBlob(response, '尿酸数据')
      })
    }
  }
}
