...
export default {
  ...
  created () {
    ...
    rolePermission({ roleId: this.roleId }).then(res => {
      this.info = this.formatData(this.dealData(res.data), [])
    })
  },
  methods: {
    /**
    ...
    formatData (data, parent) {
      ...
      for (let i = 0; i < data.length; i++) {
        data[i].parent = [...parent, data[i].id]
        arr.push(data[i])
        if (data[i].children.length > 0) {
          this.formatData(data[i].children, data[i].parent)
        }
      }
      return arr
    },
    ...
    dealParams () {
      ...
      this.info.forEach(item => {
        if (item.checked == '2') {
          this.permissionData.push(item.parent)
        }
        if (item.children) {
          item.children.forEach(second => {
            if (second.checked == '2') {
              this.permissionData.push(second.parent)
            }
            // 三级
            if (second.children) {
              second.children.forEach(third => {
                if (third.checked == '2') {
                  this.permissionData.push(third.parent)
                }
                // 四级
                if (third.children) {
                  third.children.forEach(fourth => {
                    if (fourth.checked == '2') {
                      this.permissionData.push(fourth.parent)
                    }
                    // 五级
                    if (fourth.children) {
                      fourth.children.forEach(fif => {
                        if (fif.checked == '2') {
                          this.permissionData.push(fif.parent)
                        }
                      })
                    }
                  })
      ...
      this.formatRes(this.permissionData)
    },
    /**
     * 格式化结果
     * 1.数组组合
    ...
    async submit () {
      ...
      try {
        ...
        await rolePermissionStore({
          roleId,
          menuIds: this.formatRes(permissionData)
        })
        this.loading = false
        this.cancel()
      } catch (e) {
