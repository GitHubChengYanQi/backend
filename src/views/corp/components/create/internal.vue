    ...
    <div class="flex" style="align-items: start">
      <a-card class="left-card">
        ...
        <div class="step-1">
          ...
          <div class="step-name" :class="[stepIndex==2&&showStep==1?'stepName':'']">1.创建自建应用</div>
          <div class="step-name" :class="[stepIndex==2&&showStep==2?'stepName':'']">2.输入自建应用ID和Secret</div>
          <div class="step-name" :class="[stepIndex==2&&showStep==3?'stepName':'']">3.配置可调用的应用</div>
        </div>
        <!--      3-->
        <div class="step-1">
      ...
      <a-card class="right-card">
        ...
        <a-divider/>
        <div class="foot-btn">
          <a-button type="primary" @click="saveBtn">保存，进入下一步</a-button>
          <a-button
            type="primary"
...
import { storeApi, agentStoreApi, userSecretStoreApi, callbackInfoApi, contactSecretStoreApi, agentInfoApi } from '@/api/corp'
    ...
    saveBtn () {
      ...
      if (this.stepIndex == 2) {
        ...
        if (this.showStep == 2) {
          ...
          agentStoreApi(params).then((res) => {
            this.step2Data.agentId = res.data.agentId
            this.showStep++
            agentInfoApi({
              corpId: this.corpId,
              type: 1
