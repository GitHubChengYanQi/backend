        ...
        <div class="right">
          ...
          <a-form class="form-two" :label-col="{ span: 3 }" :wrapper-col="{ span: 20 }">
            ...
            <a-form-item label="添加群聊" class="form-item">
              <a-row>
                <p>
                  可添加多个群聊，当前面的群人数达到上限后，自动发送后面的群二维码。即当第一个群达到人数上限，客户自动扫码会添加第二个员工，然后进入第二个群聊，依次往后进行。因系统无法判断群二维码是否真实有效，请您准确上传群对应的群二维码，否则会影响拉群效果
                </p>
              </a-row>
              <a-row>
                <a-table
...
const columns = [
  {
    ...
    customRender: (test, record) => {
      ...
      return `${record.roomName} (${record.num}/${record.roomMax})`
    }
  },
  {
    title: '群人数上限',
...
export default {
  ...
  methods: {
    ...
    chooseChat () {
      ...
      this.getRoomList()
      this.groupChatTable = deepClonev2(this.rooms)
    },
    // 群聊 确定
    addNewChat () {
