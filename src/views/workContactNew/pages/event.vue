<template>
  <div class="box">
    <div class="card mb16 filter-box">
      <div class="filter-input-row flex">
        <div class="item">
        ...
        <div class="item">
          ...
          <a-select placeholder="请选择事件类型" v-model="filter.form.event">
        ...
        <div class="item" v-if="filter.form.event === '7'">
          <div class="title">事件：</div>
          <a-select placeholder="请选择事件" v-model="filter.form.templateId">
            <a-select-option value="">
              全部
            </a-select-option>
            <a-select-option v-for="v in temps" :key="v.id" :value="v.id">
              {{ v.name }}
            </a-select-option>
          </a-select>
        </div>
        <div class="item" v-if="filter.form.event === '1'">
          <div class="title">选择员工：</div>
          <SelectEmployeeInput v-model="filter.form.employeeIds" :changeId="true"/>
        </div>
        <div class="item ml16">
    ...
    <div class="card">
      ...
      <div class="table-content mt23">
        ...
        >
          ...
          <div slot="event" slot-scope="row">
            {{ row.content.title }}：{{ row.content.content }}
            <a
              @click="showEvent(row)"
              class="ml3"
            >
          ...
          <div slot="operation" slot-scope="row">
            <a @click="$router.push(`/workContact/contactFieldPivot?id=${row.contactId}`)">详情</a>
          </div>
        </a-table>
...
<script>
import { synContact, templateList, trackContactList } from '@/api/workContact'
import event from '@/views/workContactNew/components/event'
...
export default {
  ...
  methods: {
    ...
    showEvent (row) {
      ...
      if (this.filter.form.time[0]) {
        time = [
          this.filter.form.time[0].format('YYYY-MM-DD'),
          this.filter.form.time[1].format('YYYY-MM-DD')
        ]
      }

      this.$refs.event.show(row.contactId, ev, userInfo, time)
    },
    ...
    exportData () {
      ...
      if (filter.time.length) {
        filter.startTime = filter.time[0]
        filter.endTime = filter.time[1]
      }
  ...
  components: { event, addTag, removeTag }
}
</script>
