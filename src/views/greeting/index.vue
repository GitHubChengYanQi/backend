<template>
  <div class="wrapper-box">
    ...
    <a-card>
      <div class="btn-box">
        <router-link
          :to="{path: `/greeting/store?hadGeneral=${hadGeneral}&hadEmployees=${hadEmployees}`}"
        >
      ...
      >
        <div slot="words" slot-scope="text, record">
          <template>
            <div v-if="record.words !== ''">文本：{{ record.words }}</div>
            <div v-if="record.mediumContent.appid != undefined">
              小程序：
              <a-icon type="link" />
              <span>{{ record.mediumContent.title }}</span>
            </div>
            <div
              class="img-box"
              v-if="record.mediumContent.appid == undefined && record.mediumContent.title != undefined"
            >
              <span>链接：</span>
              <div class="text-box">
                ...
                <div>
                  <span
                    v-if="record.mediumContent.description"
                  >{{ record.mediumContent.description }}</span>
        ...
        <div slot="employees" slot-scope="text, record" class="employeesBox">
          <template>
            <a-popover title="适用成员" v-if="record.employees.length > 0">
              <template slot="content">
                <div class="labelBox">
                  <a-tag v-for="(item, index) in record.employees" :key="index">{{ item }}</a-tag>
                </div>
              </template>
              <a-tag type="button">
                查看
        ...
        <div slot="action" slot-scope="text, record">
          <template>
            <router-link
              :to="{path:`/greeting/store?greetingId=${record.greetingId}&hadGeneral=${hadGeneral}&hadEmployees=${hadEmployees}`}"
            >
              <a-button v-permission="'/greeting/index@edit'" type="link">编辑</a-button>
            </router-link>
            <a-button
              v-permission="'/greeting/index@delete'"
              type="link"
              @click="showDeleteConfirm(record.greetingId)"
            >删除</a-button>
          </template>
        </div>
      </a-table>
...
export default {
  ...
  methods: {
    ...
    openEmployeesToast (index) {
      console.log(index, 'index')
      const obj = {
        show: true,
        index: this.tableEmplyeesToast.index === index ? -1 : index
      }
      this.tableEmplyeesToast = { ...obj }
    }
  }
}
</script>
