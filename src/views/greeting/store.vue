  ...
  <div class="wrapper">
    <div class="left">
      <a-card title="客户账号">
        <div class="dialogue-box">
          ...
          <div class="content">{{ addWelcomeData.words }}</div>
        </div>
            ...
            <a-row>
              <a-col :lg="8">
                <a-radio-group v-model="addWelcomeData.rangeType">
                  <a-radio :disabled="isHadGeneral" :value="1">通用</a-radio>
                  <a-radio :default-checked="isHadGeneral" :value="2">指定企业成员</a-radio>
                </a-radio-group>
              </a-col>
            </a-row>
            ...
            <a-row v-if="addWelcomeData.rangeType === 2">
              <a-col>
                <div v-if="employeeNum != 0">
                  ...
                  <a-button
                    type="link"
                    @click="() => {this.employeeIdList = ''; this.employees = [];this.employeeNum = 0}"
                  >重置</a-button>
                </div>
              </a-col>
            </a-row>
          ...
          <a-form-model-item label="添加内容：" v-if="mediumTitle === ''">
            <a-popover>
              <template slot="content">
                <a-button
                  icon="file-image"
                  @click="() => {this.visible = true; this.mediumType = 2; this.getMaterialLibrary('img')}"
                >图片</a-button>
                <!-- <a-button
                  icon="link"
                  @click="() => {this.imageTextModal = true; this.mediumType = 3; this.isImageText = true}"
                >图文</a-button> -->
                <a-button
                  icon="paper-clip"
                  @click="() => {this.appletsModal = true; this.mediumType = 6; this.isApplets = true}"
                >小程序</a-button>
              </template>
              <!-- 隐藏图文 -->
              <a-button>+添加图片/小程序</a-button>
            </a-popover>
          </a-form-model-item>
          <a-form-model-item v-else class="medium-btn">
            <a-icon type="link" />
            <span @click="showMedium">{{ mediumTitle }}</span>
            <a-icon type="close" @click="closeMedium" />
          </a-form-model-item>
          <template>
            <a-button
              v-permission="'/greeting/store@add'"
              style="marginLeft: 50px"
              type="primary"
              :loading="btnLoading"
              @click="addWelcomeMesage"
            >{{ greetingId ? '保存':'创建欢迎语' }}</a-button>
          </template>
        </a-form-model>
      </a-card>
    </div>
    ...
    >
      ...
      <template slot="footer">
        <a-button
          @click="() => { this.choosePeopleShow = false; this.employeeIdList = ''; this.employees = [] }"
        >取消</a-button>
        ...
        >
          ...
          <div v-if="isImport === 0">
            ...
            <a-form-model-item label="添加描述">
              <a-textarea
                :rows="4"
                v-model="imageTextData.description"
                placeholder="填写图文描述"
                :maxLength="128"
              ></a-textarea>
            </a-form-model-item>
            <a-form-model-item label="素材同步">
              ...
              <a-select class="select-box" v-model="mediumGroupId" v-if="isSync == 1">
                <a-select-option v-for="item in mediumGroupList" :key="item.id">{{ item.name }}</a-select-option>
              </a-select>
            </a-form-model-item>
            <a-form-model-item label="点击跳转" prop="imageLink">
              <a-input
                v-model="imageTextData.imageLink"
                placeholder="请输入跳转链接，且必须以http://或http://开头"
              ></a-input>
            </a-form-model-item>
          </div>
          <div v-else>
            <a-form-model :label-col="{ span: 4 }" :wrapper-col="{ span: 18}">
              <a-form-model-item label="图片">
                ...
                <div class="import-box" v-else>
                  ...
                  <a-button
                    type="link"
                    @click="() => {this.imageTextModal = false; this.visible = true}"
                  >[修改]</a-button>
                </div>
              </a-form-model-item>
            </a-form-model>
    ...
    <!-- 添加素材库弹窗 -->
    <a-modal v-model="visible" centered @ok="handleAddLibraryOk" width="95%">
      <MediumGroup :is-component="true" v-if="visible" @materialSelect="librarySelectChange" />
    </a-modal>
      ...
      >
        ...
        <div class="picture-box" v-if="mediumType === 2">
          ...
          <div
            ...
            :key="item.id"
            @click="definedMedium(item.id, item.content)"
          >
    ...
    >
      <a-form-model :label-col="{ span: 4 }" :wrapper-col="{ span: 18}">
        <a-form-model-item label="上传图片">
          <upload :imageUrl="imgUrl" @success="uploadSuccess" :file-type="1"></upload>
      ...
      <template slot="footer">
        <a-button
          @click="() => { this.localUploadModal = false; this.visible = true; this.isSync = 1; this.imgUrl = '' }"
        >取消</a-button>
          ...
          <a-form-model :label-col="{ span: 4 }" :wrapper-col="{ span: 18}">
            <a-form-model-item label="小程序">
              ...
              <div class="import-box" v-else>
                ...
                <a-button
                  type="link"
                  @click="() => {this.appletsModal = false; this.visible = true}"
                >[修改]</a-button>
              </div>
            </a-form-model-item>
          </a-form-model>
...
export default {
  ...
  data () {
    return {
      ...
      selectList: [],
      // 素材库选择
      libraryType: '', // img-图片 linkImg-图文图片 link-图文 miniImg-小程序图片 mini-小程序
      librarySelects: []
    }
  },
  computed: {
  ...
  methods: {
    ...
    getGreetingDetail () {
      greetingDetail({
        ...
        const ids = this.customTrim(res.data.employees, 2)
        this.employees = ids.split(',')
        this.peopleChange(res.data.employees.split(','))
    ...
    // 获取素材库
    async getMaterialLibrary (type, init = true) {
      if (init) {
        await this.getMediumGroup()
      }
      if (type) {
    ...
    // 素材库分组改变
    changeGroup (value, options) {
      this.mediumGroupId = value
      this.getMaterialLibrary(null, false)
    },
    // 选择图片
    mediumModal (type) {
      this.visible = true
      ...
      this.getMaterialLibrary(type === 'image' ? 'linkImg' : type === 'applets' ? 'miniImg' : null)
    },
    // 点击素材
    ...
    importMedium (type) {
      ...
      this.getMaterialLibrary(type === '图文' ? 'link' : 'mini')
    },
    // 导入选择
      ...
      if (this.isImport == 1) {
        ...
        this.$refs.imageForm.validate(valid => {
          if (valid) {
            ...
            this.addMaterLibraryItem('link', content)
          } else {
      ...
      if (this.isImport == 1) {
        ...
        this.$refs.appletsForm.validate(valid => {
          if (valid) {
            ...
            this.addMaterLibraryItem('mini', content)
          } else {
            console.log('error submit!!')
            return false
          }
    ...
    // 添加到欢迎语中
    addMaterLibraryItem (type, content, isSync = this.isSync) {
      if (isSync === 2 || ['link', 'mini'].includes(type)) {
        this.mediumType = type === 'link' ? 3 : 6
        this.mediumId = content.id
        this.mediumDetail = content
        this.mediumDetail.imageFullPath = this.upLoadRes.imageFullPath
        this.mediumDetail.type = type === 'link' ? 3 : 6
        this.mediumTitle = content.title
        if (type === 'link') {
          this.imageTextModal = false
        } else {
    ...
    librarySelectChange (e) {
      this.librarySelects = e
    },
    // 素材库弹窗确认
    handleAddLibraryOk () {
      if (!this.librarySelects.length) {
        this.$message.error('请选择素材')
        return
      } else if (this.librarySelects.length !== 1) {
        this.$message.error('只能选择一个素材')
        return
      }
      const libraryItem = this.librarySelects[0]
      const type = this.libraryType
      console.log(type, libraryItem)
      if (['img', 'linkImg', 'miniImg'].includes(type) && libraryItem.type_id !== 2) {
        this.$message.error('请选择图片素材')
        return
      } else if (type === 'link' && libraryItem.type_id !== 3) {
        this.$message.error('请选择图文素材')
        return
      } else if (type === 'mini' && libraryItem.type_id !== 6) {
        this.$message.error('请选择小程序素材')
        return
      }
      const { content, id } = libraryItem
      if (type === 'img') {
        this.mediumId = id
        this.upLoadRes = content
        this.mediumTitle = content.imageName
        ...
        this.upLoadRes.imageFullPath = content.imageFullPath
        this.upLoadRes.imageName = content.imageName
        this.addMaterLibraryItem(type, linkObj, 2)
      } else if (type === 'mini') {
        const linkObj = {
          title: content.title,
          page: content.page,
