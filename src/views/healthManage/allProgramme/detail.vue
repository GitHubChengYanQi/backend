...
<template>
  <a-card>
    ...
    <div class="btns_">
      <Space>
        <a-button type="primary" @click="addProgramme" v-permission="'/planTemplate/addToPlan#post'">
          添加到随访方案
        </a-button>
...
export default {
  ...
  methods: {
    // 查看详情
    async getDetail () {
      const { data } = await planTemplateDetail({ planId: this.planId })
      this.pageData = data
    },
    // 点击问卷查询详情
    ...
    async suspendOrContinue () {
      ...
      if (status == 0) {
        console.log('启用')
        data = await planTemplateEdit({ planId, status: 1 })
      } else {
        console.log('暂停')
        data = await planTemplateEdit({ planId, status: 0 })
      }
      if (data.code == 200) {
    ...
    addProgramme () {
      this.$confirm({
        ...
        cancelText: '取消',
        onOk: async () => {
          const { code } = await planTemplateAddToPlan({ planId: this.planId })
          if (code == 200) {
            this.$message.success('操作成功')
          } else {
            this.$message.error('操作失败')
