    ...
    <a-modal
      ...
      <a-form-model ref="ruleForm" :model="addAttribute" :rules="rules" :label-col="{ span: 5 }" :wrapper-col="{ span: 17 }">
        ...
        <a-form-model-item label="排序展示：">
          <a-input-number v-model="addAttribute.order"></a-input-number>
        </a-form-model-item>
    ...
    <a-modal
      ...
      <a-form :label-col="{ span: 5 }" :wrapper-col="{ span: 17 }">
        ...
        <a-form-item label="排序展示：">
          <a-input-number style="width: 100%" :min="0" v-model="editDetail.order"></a-input-number>
    ...
    submit () {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          if (params.type == 1) {
            const statusOption = params.options.every(i => {
              if (i.length <= 12) {
                return true
              }
            })
            if (!statusOption) {
              this.$message.warning('新增选项内容,每个选项内容不超过12个字')
              return
            }
          }
          this.btnLoading = true
