<template>
  <div class="wrapper">
    <div
      ...
      <a-alert
        :show-icon="false"
        message="1、可以生成带参数的二维码名片，支持活码功能，即随机选取设置的活码成员推给用户。加企业微信为好友后，可以给微信联系人自动回复相应欢迎消息和打标签。"
        banner />
      <a-alert
        :show-icon="false"
        message="2、每创建一个渠道活码，该码则自动进入【内容引擎】--【图片类型】--分组【渠道码-企业微信】，以素材的方式通过聊天侧边栏快速发送给客户。"
        banner />
      <a-alert
        :show-icon="false"
        message="3、受限于官方，单人类型的渠道码创建后尽量不要再修改成员，否则会造成列表中，该二维码中间的头像与配置的成员头像不一致，但是并不影响功能使用。"
        banner />
      <a-alert
        :show-icon="false"
        message="4、如果企业在企业微信后台为相关成员配置了可用的欢迎语，使用第三方系统配置欢迎语，则均不起效，推送的还是企业微信官方的。"
        banner />
    </div>
    <a-card>
      <a-form
        ...
        <a-row :gutter="16">
          <a-col :lg="8">
            <a-form-item label="名称：">
              <a-input
                v-model="screentData.name"
                placeholder="搜索活码名称"></a-input>
            </a-form-item>
          </a-col>
          <a-col :lg="8">
      ...
      <div class="search">
        <a-button
          v-permission="'/channelCode/index@search'"
          type="primary"
      ...
      <div class="btn-box">
        <a-button
          v-permission="'/channelCode/index@editGroup'"
          type="primary"
          @click="() => {this.editGroupDis = true}">修改分组</a-button>
        <a-button
          v-permission="'/channelCode/index@add'"
          type="primary"
          @click="() => {this.addGroupDis = true}">新建分组</a-button>
        <router-link :to="{path: '/channelCode/store'}">
          <a-button
            v-permission="'/channelCode/store'"
        ...
        <div
          ...
          <template>
            <router-link :to="{path:`/channelCode/store?channelCodeId=${record.channelCodeId}`}">
              <a-button
                v-permission="'/channelCode/index@edit'"
                type="link">编辑</a-button>
            </router-link>
            <!-- <a-button type="link">编辑</a-button> -->
            <a-button
              v-permission="'/channelCode/index@customer'"
              type="link"
              @click="getChannelCodeContact(record.channelCodeId)">客户</a-button>
            <a-button
              v-permission="'/channelCode/index@download'"
              type="link">
              <a
            ...
            <a-button
              v-permission="'/channelCode/index@move'"
              type="link"
              @click="moveGroup(record.channelCodeId, record.groupId)">移动</a-button>
            <router-link :to="{path: `/channelCode/statistics?channelCodeId=${record.channelCodeId}`}">
              <a-button
                v-permission="'/channelCode/statistics'"
                type="link">统计</a-button>
            </router-link>
          </template>
        </div>
      </a-table>
    ...
    <a-modal
      ...
      <template slot="footer">
        <a-button
          type="primary"
          @click="() => {this.contactModal = false; this.contactPagination.current = 1;}">确定</a-button>
      </template>
    </a-modal>
    <a-modal
      title="新建分组"
    ...
    <a-modal
      ...
      <div class="group-box">
        <div
          :class="moveGroupId == item.groupId ? 'active' : ''"
          v-for="item in groupList"
          :key="item.groupId"
      ...
      <template slot="footer">
        ...
        <a-button
          type="primary"
          @click="moveGroupDefined">确定</a-button>
      </template>
    </a-modal>
  </div>
</template>
