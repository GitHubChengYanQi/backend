<template>
  <div id="userSop_Page_Container" ref="userSop_Page_Container">
    <topHeader
      :searchPermission="'/sopRegular/index@get'"
      :addPermission="'/sopRegular/add@post'"
      ...
      <div slot="options" slot-scope="text, record">
        <template>
          <div style="display: flex;justify-content: space-around;">
            <a-button type="link" @click="goExportData(record)" v-permission="'/sopTaskDetail/getSopTaskDetailList@post'">数据</a-button>
            <a-button type="link" @click="editItem(record)" v-permission="'/sopRegular/detail@get'">编辑</a-button>
            <a-button type="link" v-permission="'/sopRegular/delete@post'" @click="deleteItem(record.id)">删除</a-button>
          </div>
        </template>
      </div>
    </a-table>
...
import { getRegularList, delRegularSop, updateRegularState } from '@/api/salers'
export default {
  data () {
    return {
      // 数据弹框传入对象
  ...
  created () {
    const tempPageIndex = sessionStorage.getItem('regularPage')
  ...
  methods: {
    ...
    goExportData (record) {
      ...
      this.$set(tempInfo, 'sopType', 'Regular')
      ...
      // this.$set(this.exportDataObj, 'sopType', 'Regular')
    ...
    getDataList ({ page = 1, perPage = 10, sopName = '' }) {
      ...
      getRegularList(params).then(response => {
    ...
    editItem (info) {
      ...
      // sessionStorage.setItem('activeType', 'Regular')
      sessionStorage.setItem('regularPage', this.pageIndex)
      this.$router.push({
    ...
    deleteItem (id) {
      ...
      this.$confirm({
        ...
        cancelText: '取消',
        onOk: async () => {
          delRegularSop(params).then(response => {
    ...
    async sopItemChange (item, index) {
      ...
      await updateRegularState(params).then(response => {
        if (response.code === 200) {
