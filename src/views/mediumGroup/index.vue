  ...
  <div class="material-library">
    <a-row>
      <a-col>
        ...
        >
          <a-form-model-item label="选择父级分组：">
            <a-tree-select
              v-model="groupPid"
              style="width: 100%"
        ...
        <a-modal
          ...
          @cancel="() => {this.editGroupModal = false; this.editGroupPid = ''; this.editGroupId = ''; this.editGroupName = ''}"
        >
          <a-form-model :label-col="{ span: 5 }" :wrapper-col="{ span: 14 }">
            <a-form-model-item label="选择分组">
              <a-tree-select
                ...
                @change="update_tree_change"
                :treeData="treeData">
              </a-tree-select>
            </a-form-model-item>
            <a-form-model-item label="选择父级分组">
              <a-tree-select
                v-model="editGroupPid"
                style="width: 100%"
                :dropdown-style="{ maxWidth: '400px', maxHeight: '400px', overflow: 'auto' }"
                placeholder="请选择分组"
                allow-clear
          ...
          <template slot="footer">
            <a-button key="back" @click="() => {this.editGroupModal = false; this.editGroupPid = ''; this.editGroupId = ''; this.editGroupName = ''}">
              取消
            </a-button>
        ...
        <a-modal
          title="删除分组"
          :visible="deleteGroupModel"
          @cancel="() => {this.deleteGroupModel = false; this.deleteGroupId = '';}"
        >
          <a-form-model-item label="选择分组：">
            <a-tree-select
          ...
          <template slot="footer">
            <a-button key="back" @click="() => {this.deleteGroupModel = false;this.deleteGroupId = '';}">
              取消
            </a-button>
            <a-button :loading="btnLoading" key="submit" @click="delGroup" type="primary">
              确定
            </a-button>
          </template>
        </a-modal>
        <a-card class="tag-box">
          ...
          <div class="group-btn" v-if="!isComponent">
            <a-button v-permission="'/mediumGroup/index@destroyGroup'" type="primary" @click="() => {this.deleteGroupModel = true}" style="marginRight: 10px">删除分组</a-button>
            ...
            <a-button v-permission="'/mediumGroup/index@addGroup'" type="primary" @click="addGroupModelShow">新增分组</a-button>
          </div>
          <a-tabs @change="changeTab">
            <a-tab-pane :key="0" :tab="isComponent ? '素材库' : '所有'">
              <p v-if="!isComponent">共有{{ pagination.total }}个素材</p>
              ...
              <a-table
                ...
                :row-selection="isComponent ? { selectedRowKeys: selectedRowKeys, onChange: onSelectChange } : null"
                @change="handleTableChange">
                <div slot="titles" slot-scope="text, record">
                  <span v-if="record.type === '文本' || record.type === '视频' || record.type === '音频' || record.type === '图片'|| record.type === '文件'">{{ record.content.title }}</span>
                  <span v-if="record.type === '图文' || record.type === '小程序' ">{{ record.content.maintitle }}</span>
                </div>
                ...
                <div slot="action" slot-scope="text, record" v-if="!isComponent">
                  <template>
                    <a-button v-permission="'/mediumGroup/index@edit'" type="link" v-if="record.type == '图片' || record.type == '文本' || record.type == '图文' || record.type == '小程序'" @click="edit(record.id, record.type)">编辑</a-button>
                    <a-button v-permission="'/mediumGroup/index@move'" type="link" @click="moveTagModal(record.id)">移动</a-button>
                    <a-popconfirm
                      v-permission="'/mediumGroup/index@delete'"
                      title="确认删除该素材？"
                      ok-text="确认"
                      cancel-text="取消"
            ...
            <a-tab-pane v-if="!isComponent" key="1" tab="文本">
              ...
              <div class="top-btn">
                ...
                <div class="btn">
                  <a-button type="primary" v-permission="'/mediumGroup/index@add'" style="marginRight: 10px" @click="() => {this.textModal = true;}">+添加文本</a-button>
                </div>
              </div>
              <a-table
                style="marginTop:20px;"
                ...
                <div slot="contents" slot-scope="text, record" style="width:100%;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">
                  {{ record.content.content }}
                </div>
                <div slot="action" slot-scope="text, record">
                  <template>
                    <a-button v-permission="'/mediumGroup/index@edit'" type="link" @click="editDetail(record.id, 1)">编辑</a-button>
                    <a-button v-permission="'/mediumGroup/index@move'" type="link" @click="moveTagModal(record.id)">移动</a-button>
                    <a-popconfirm
                      v-permission="'/mediumGroup/index@delete'"
                      title="确认删除该素材？"
            ...
            <a-tab-pane v-if="!isComponent" key="2" tab="图片">
              <p>共有{{ pagination.total }}个素材</p>
              <div class="top-btn">
                <div class="search">
                  <a-input-search
              ...
              <div class="picture">
                <a-card style="width: 250px" v-for="item in pictureTableList" :key="item.id">
                  ...
                  <div class="text-box">
                    <span>标题： {{ item.content.title }}</span>
                    <span>上传者： {{ item.userName }}</span>
                    <span>来源：{{ item.mediumGroupName }}</span>
                  </div>
                  <template slot="actions" class="ant-card-actions">
                    <a-button v-permission="'/mediumGroup/index@edit'" type="link" @click="editDetail(item.id, 2)">编辑</a-button>
                ...
                <div v-if="pictureTableList.length === 0" style="width:100%">
              ...
              <a-pagination :current="pagination.current" :total="pagination.total" show-size-changer @showSizeChange="onShowSizeChange" @change="handlePageChange" />
            </a-tab-pane>
            <a-tab-pane v-if="!isComponent" key="3" tab="图文">
              <p>共有{{ pagination.total }}个素材</p>
              <div class="top-btn">
                <div class="search">
                  <a-input-search
                ...
                <div class="btn">
                  <a-button v-permission="'/mediumGroup/index@add'" type="primary" style="marginRight: 10px" @click="() => { this.imageTextModal = true; this.modalType = 3 }"><a-icon type="plus" />添加图文</a-button>
                </div>
              </div>
              <div class="picture">
                <a-card style="width: 250px" v-for="item in pictrueTextList" :key="item.id">
                  ...
                  <div class="text-box">
                    <span>标题： {{ item.content.maintitle }}</span>
                    <span>上传者： {{ item.userName }}</span>
                    <span>来源：{{ item.mediumGroupName }}</span>
                  </div>
                ...
                <div v-if="pictrueTextList.length === 0" style="width:100%">
                  <div class="ant-empty ant-empty-normal">
                    <div class="ant-empty-image">
                      <svg width="64" height="41" viewBox="0 0 64 41" xmlns="http://www.w3.org/2000/svg">
                        <g transform="translate(0 1)" fill="none" fillRule="evenodd">
            ...
            <a-tab-pane v-if="!isComponent" key="4" tab="音频">
              <p>共有{{ pagination.total }}个素材</p>
              <div class="top-btn">
                <div class="search">
                  <a-input-search
                ...
                <div class="btn">
                  <a-button type="primary" v-permission="'/mediumGroup/index@add'" style="marginRight: 10px" @click="modalTypeDefine(4)"><a-icon type="upload" />上传音频</a-button>
                </div>
              </div>
              <div class="picture">
                ...
                <div v-if="voiceTableList.length === 0" style="width:100%">
                  <div class="ant-empty ant-empty-normal">
                    <div class="ant-empty-image">
                      <svg width="64" height="41" viewBox="0 0 64 41" xmlns="http://www.w3.org/2000/svg">
                        <g transform="translate(0 1)" fill="none" fillRule="evenodd">
            ...
            <a-tab-pane v-if="!isComponent" key="5" tab="视频">
              <p>共有{{ pagination.total }}个素材</p>
              <div class="top-btn">
                <div class="search">
                  <a-input-search
                ...
                <div class="btn">
                  <a-button v-permission="'/mediumGroup/index@add'" type="primary" style="marginRight: 10px" @click="modalTypeDefine(5)"><a-icon type="upload" />上传视频</a-button>
                </div>
              </div>
              <div class="picture">
                ...
                <div v-if="videoList.length === 0" style="width:100%">
                  <div class="ant-empty ant-empty-normal">
                    <div class="ant-empty-image">
                      <svg width="64" height="41" viewBox="0 0 64 41" xmlns="http://www.w3.org/2000/svg">
                        <g transform="translate(0 1)" fill="none" fillRule="evenodd">
            ...
            <a-tab-pane v-if="!isComponent" key="6" tab="小程序">
              <p>共有{{ pagination.total }}个素材</p>
              <div class="top-btn">
                <div class="search">
                  <a-input-search
                ...
                <div class="btn">
                  <a-button v-permission="'/mediumGroup/index@add'" type="primary" style="marginRight: 10px" @click="() => { this.appletsModal = true; this.modalType = 6 }"><a-icon type="plus" />添加小程序</a-button>
                </div>
              </div>
              <div class="picture">
                <a-card style="width: 250px" v-for="item in appletsTableList" :key="item.id">
                  ...
                  <template slot="actions" class="ant-card-actions">
                    <a-button v-permission="'/mediumGroup/index@edit'" type="link" @click="editDetail(item.id, 6)">编辑</a-button>
                    <a-button v-permission="'/mediumGroup/index@move'" type="link" @click="moveTagModal(item.id)">移动</a-button>
                    <a-popconfirm
                      v-permission="'/mediumGroup/index@delete'"
                      title="确认删除该素材？"
                ...
                <div v-if="appletsTableList.length === 0" style="width:100%">
                  <div class="ant-empty ant-empty-normal">
                    <div class="ant-empty-image">
                      <svg width="64" height="41" viewBox="0 0 64 41" xmlns="http://www.w3.org/2000/svg">
                        <g transform="translate(0 1)" fill="none" fillRule="evenodd">
            ...
            <a-tab-pane v-if="!isComponent" key="7" tab="文件">
              <p>共有{{ pagination.total }}个素材</p>
              <div class="top-btn">
                <div class="search">
                  <a-input-search
                ...
                <div class="btn">
                  <a-button v-permission="'/mediumGroup/index@add'" type="primary" style="marginRight: 10px" @click="modalTypeDefine(7)"><a-icon type="upload" />上传文件</a-button>
                </div>
              </div>
              <div class="picture">
                ...
                <div v-if="fileTableList.length === 0" style="width:100%">
                  <div class="ant-empty ant-empty-normal">
                    <div class="ant-empty-image">
                      <svg width="64" height="41" viewBox="0 0 64 41" xmlns="http://www.w3.org/2000/svg">
                        <g transform="translate(0 1)" fill="none" fillRule="evenodd">
          ...
          <a-modal
            :visible="textModal"
            :title="editTypeNum === '' ? '新建文本素材':'修改文本素材'"
            @cancel="() => {this.$refs.textForm.resetFields(); this.editTypeNum =''; this.textModal = false; this.addTextData.content.title = ''; this.addTextData.content.content = ''; this.materialGroupId = 0 }">
            <a-form-model ref="textForm" :model="addTextData.content" :rules="textRules" :label-col="{ span: 5 }" :wrapper-col="{ span: 15 }">
              ...
              <a-form-model-item label="文本标题：" prop="title">
                <a-input v-model="addTextData.content.title" :maxLength="15" />
              </a-form-model-item>
              <a-form-model-item label="文本内容：" prop="content">
                <mtextarea
                  v-model="addTextData.content.content"
                  :showWordLimit="true"
                  :maxLength="1000"
                  :autoSize="false"
                  placeholder="请输入描述"/>
                <!-- <a-textarea maxlength=1000 v-model="addTextData.content.content"></a-textarea> -->
              </a-form-model-item>
            </a-form-model>
            <template slot="footer">
              <a-button @click="() => {this.$refs.textForm.resetFields(); this.editTypeNum =''; this.textModal = false; this.addTextData.content.title = ''; this.addTextData.content.content = ''; this.materialGroupId = 0 }">取消</a-button>
          ...
          <a-modal
            ...
            title="编辑文本"
            @cancel="() => {this.editTextModal = false; this.editTypeNum = ''}">
            <a-form-model :label-col="{ span: 5 }" :wrapper-col="{ span: 15 }">
              <a-form-model-item label="选择分组：">
                <a-tree-select
                  v-model="materialDetail.mediumGroupId"
                  style="width: 100%"
                  :dropdown-style="{ maxWidth: '400px', maxHeight: '400px', overflow: 'auto' }"
                  placeholder="请选择分组"
                  allow-clear
            ...
            <template slot="footer">
              <a-button @click="() => {this.editTextModal = false; this.editTypeNum = ''}">取消</a-button>
              <a-button type="primary" @click="addText">确定</a-button>
            </template>
          </a-modal>
          <a-modal
            :visible="imageTextModal"
            :title="this.editTypeNum === '' ? '新建图文素材': '修改图文素材'"
          ...
          <a-modal
            ...
            title="编辑图文"
            @cancel="() => {this.editImageTextModal = false; this.editTypeNum =''; this.imgUrl = ''; this.upLoadRes = {}}">
            <a-form :label-col="{ span: 5 }" :wrapper-col="{ span: 17 }">
              ...
              <a-form-model-item label="标题名称：">
                <a-input v-model="materialDetail.content.maintitle" />
              </a-form-model-item>
            ...
            <template slot="footer">
              <a-button @click="() => {this.editImageTextModal = false; this.editTypeNum =''; this.imgUrl = ''; this.upLoadRes = {}}">取消</a-button>
              <a-button type="primary" @click="addImageTextDefine">确定</a-button>
            </template>
          </a-modal>
          <a-modal
            ...
            title="编辑图片"
            @cancel="() => {this.editImageModal = false; this.editTypeNum =''; this.imgUrl = ''; this.upLoadRes = {}}">
            <a-form :label-col="{ span: 5 }" :wrapper-col="{ span: 17 }">
              <a-form-item label="选择分组：">
                <a-tree-select
                  v-model="materialDetail.mediumGroupId"
              ...
              <a-form-model-item label="图片名称：">
                <a-input v-model="materialDetail.content.imageName" />
              </a-form-model-item>
            ...
            <template slot="footer">
              <a-button @click="() => {this.editImageModal = false; this.editTypeNum =''; this.imgUrl = ''; this.upLoadRes = {}}">取消</a-button>
              <a-button type="primary" @click="addPhoto">确定</a-button>
            </template>
          </a-modal>
          <a-modal
            :visible="addPhotoModal"
            :title="this.editTypeNum === '' ? '新建图片素材': '修改图片素材'"
            @cancel="() => {this.$refs.addPhotoForm.resetFields(); this.photoData = {}; this.addPhotoModal = false; this.editTypeNum =''; this.imgUrl = ''; this.upLoadRes = {}}">
            <a-form-model
              :model="photoData"
              :rules="addPhotoForm"
              ref="addPhotoForm"
            ...
            <template slot="footer">
              <a-button @click="() => {this.$refs.addPhotoForm.resetFields(); this.photoData = {}; this.addPhotoModal = false; this.editTypeNum =''; this.imgUrl = ''; this.upLoadRes = {}}">取消</a-button>
              <a-button type="primary" @click="addPhoto">确定</a-button>
            </template>
          </a-modal>
          <a-modal
          ...
          <div class="jbox" ref="jbox">
            <a-modal
              ...
              :title="this.editTypeNum === '' ? '新增小程序': '编辑小程序'"
              ...
              <div class="applets-box">
                <div class="left">
                  ...
                  <a-form-model ref="appletsForm" :model="appletsData" :rules="appletsForm" :label-col="{ span: 5 }" :wrapper-col="{ span: 15 }">
                    ...
                    <a-form-model-item label="标题名称：" prop="maintitle">
                      <a-input v-model="appletsData.maintitle" :maxLength="15" />
                    </a-form-model-item>
                    ...
                    <a-form-model-item label="小程序标题：" prop="title">
          ...
          <div class="bbox" ref="bbox">
            <a-modal
              ...
              <div class="tag-box">
                <span :class="classActive === item.id ? 'active' : 'box'" v-for="item in treeFlatData" :key="item.id" @click="moveTag(item.id)">{{ item.name }}</span>
              </div>
              <template slot="footer">
...
export default {
    ...
    return {
      ...
      imageForm: {
        ...
        imageLink: [
          ...
          { pattern: /(http|https):\/\/([\w.]+\/?)\S*/, message: '必须以http://或https://开头', trigger: 'blur' }
        ]
      },
      appletsForm: {
        maintitle: [
  ...
  methods: {
    flatTree (data, childName = 'children') {
      ...
      return clone.reduce((prev, curt) => {
        let childs = []
        if (curt[childName] && curt[childName].length > 0) {
          childs = this.flatTree(curt[childName])
        }
        delete curt[childName]
        return [...prev, curt, ...childs]
      }, [])
    },
    update_tree_change (value, label, extra) {
      this.editGroupPid = ''
      const item = this.treeFlatData.find(item => {
        return item.id === value
      })
      if (item && item.level > 0) {
        this.treeParentData = this.treeFlatData.filter(item2 => {
          return item2.level === item.level - 1 && item2.name !== '未分组'
        })
      } else {
        this.editGroupPid = ''
        this.treeParentData = []
    ...
    getTableData () {
      ...
      if (Number(this.tabsNumber) == 1) {
        params['searchStr2'] = this.searchValue2
      }
    ...
    getGroupList () {
      mediumGroup().then(res => {
        ...
        this.groupList = this.formatGroupList(res.data, [])
      })
    },
    // 重构list结构
    formatGroupList (groupList, result) {
      const self = this
      for (const i in groupList) {
        result.push(groupList[i])
        if (groupList[i].children) {
          self.formatGroupList(groupList[i].children, result)
        }
      }
      return result
    },
    ...
    async edit (id, type) {
      ...
      if (type === '文本') {
        this.editTypeNum = 1
        this.textModal = true
        this.addTextData.content = this.materialDetail.content
        this.materialGroupId = this.materialDetail.mediumGroupId
      } else if (type === '图文') {
        this.editTypeNum = 3
        // this.editImageTextModal = true
        this.modalType = 3
        this.imageTextModal = true
        this.imageTextData = this.materialDetail.content
        this.materialGroupId = this.materialDetail.mediumGroupId
        this.imgUrl = this.materialDetail.content.imageFullPath
      } else if (type === '小程序') {
        this.editTypeNum = 6
        this.modalType = 6
        // this.editAppletsModal = true
        this.appletsModal = true
        this.appletsData = this.materialDetail.content
        this.materialGroupId = this.materialDetail.mediumGroupId
        this.imgUrl = this.materialDetail.content.imageFullPath
      } else if (type === '图片') {
        this.editTypeNum = 2
        this.modalType = 2
        this.photoData = this.materialDetail.content
        ...
        this.$refs.imageForm.validate(valid => {
          if (valid) {
            ...
            const content = {
              ...
              imagePath: this.imgPath || this.imageTextData.imagePath,
              imageLink: this.imageTextData.imageLink,
              maintitle: this.imageTextData.maintitle
            }
            if (content.imagePath != undefined && content.imagePath != '') {
              editMaterialLibrary({
                id: this.materialDetail.id,
                type: this.editTypeNum,
                content: content,
        ...
        this.$refs.appletsForm.validate(valid => {
          if (valid) {
            const content = {
              ...
              maintitle: this.materialDetail.content.maintitle
            }
            editMaterialLibrary({
              id: this.materialDetail.id,
              type: this.editTypeNum,
        ...
        this.$refs.appletsForm.validate(valid => {
          if (valid) {
            const contents = {
              ...
              maintitle: this.appletsData.maintitle
            }
            if (contents.imagePath != undefined) {
              addMaterialLibrary({
                type: Number(this.tabsNumber),
    ...
    // 添加图片
    addPhoto () {
      if (this.editTypeNum === 2) {
        this.$refs.addPhotoForm.validate(valid => {
          if (valid) {
            const content = {
              title: this.materialDetail.content.title,
              imagePath: this.upLoadRes.imagePath || this.materialDetail.content.imagePath,
              imageName: this.materialDetail.content.imageName
            }
            editMaterialLibrary({
              id: this.materialDetail.id,
              type: this.editTypeNum,
    ...
    resetTextImg (type) {
      ...
      this.materialDetail.content.imagePath = ''
      if (type === '重置') {
        this.imageTextModal = true
      } else {
        this.imageTextModal = false
    ...
    onSelectChange: function (selectedRowKeys, selectedRows) {
      ...
      this.$emit('materialSelect', selectedRows)
    },
    clearSeach () {
      this.searchValue = ''
      this.searchValue2 = ''
